...
/*   Updated: 2023/09/14 20:23:34 by momox            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*identify_replace(t_data *data, char *var_env)
{
	...
	while (data->env[i++])
	{
		...
		{
			replace = ft_substr(data->env[i], ft_strlen(full_var),
...
char	*replace_env(char *arg, char *replace)
{
	...
	var = is_env(arg);
	len = env_pos(arg);
	begin = ft_substr(arg, 0, len);
	new_var = ft_strjoin(begin, replace);
	end = ft_substr(arg, var + len + 1, ft_strlen(arg));
	new_var = ft_strjoin(new_var, end);
	return (new_var);
}
